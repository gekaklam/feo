...
# SETTINGS
outfile = "generated_plots/plot_snapshot-waiting-timeline"
...
#p = p + xlab(NULL) + ylim(0, 30)

#p = p + scale_color_manual(values = c("X1m","X2m","X3m","X4m","X5m","X8m","X10m","X15m","X20m","X30m","X1h","X2h","X4h","X8h", "more"), 
#labels = c("< 1m","< 2m","< 3m","< 4m","< 5m","< 8m","< 10m","< 15m","< 20m","< 30m","< 1h","< 2h","< 4h","< 8h", "more"))
...
#p = ggplot(mdf, aes(datetime, value, fill = variable, width=500)) + geom_bar(stat='identity') + theme(legend.text=element_text(size=8), legend.key.height=unit(0.75, "line"))
p = ggplot(mdf, aes(datetime, value, fill = variable, width=15000))
#p = p + geom_bar(stat='identity') #  stacked
p = p + geom_bar(stat='identity', position="dodge") #
p = p + theme(legend.text=element_text(size=8), legend.key.height=unit(0.75, "line")) + ylab("Number of Jobs") + xlab(NULL)
...
#p = p + xlab(NULL) + ylim(0, 60)
#p = p + scale_x_datetime(breaks = date_breaks("2 days"), labels = date_format("%a"), limits = limits)
#p = p + geom_vline(data=weeks, xintercept=as.numeric(weeks$Date), linetype="longdash", size=0.2, alpha=0.5)
##p = p + theme(legend.position="bottom")
##p = p + theme_bw() 
